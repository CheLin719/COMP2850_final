...
var btn=hc.querySelector('button');if(btn)btn.addEventListener('click',function(e){e.preventDefault();var p=readSP();if(p){saveProfile(p);applyToDashboard(p);if(typeof saveHealthProfile==='function')saveHealthProfile();if(typeof showToast==='function')showToast('Profile updated','#1e6b5e');}});
...
document.getElementById('wm-nx').onclick=function(){var p=CA();saveProfile(p);applyToDashboard(p);if(typeof saveHealthProfile==='function')saveHealthProfile();var o=document.getElementById('nw-welcome-overlay');if(o){o.classList.remove('visible');setTimeout(function(){o.remove();},400);}if(typeof showToast==='function')showToast('Your plan is ready!','#1e6b5e');};
}}
